  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <Card className="border-slate-300 p-7 shadow-[8px_8px_0_rgba(89,67,49,0.08)] sm:p-9">
        <CardHeader>
          <p className="vintage-kicker mb-3">Secure client portal</p>
          <CardTitle className="text-4xl">Upload documents</CardTitle>
        ...
        <div className="mb-6 grid gap-2">
          ...
          {(items as DocumentRequestItem[] | null)?.map((item) => (
            <div key={item.id} className="flex items-center justify-between rounded-xl border border-slate-300 px-3 py-2 text-sm">
        ...
        <form action={`/api/public-upload/${token}`} method="post" encType="multipart/form-data" className="grid gap-4">
          <div className="grid gap-2">
            ...
            <select id="item_id" name="item_id" className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm">
          ...
          <div className="grid gap-2">
            ...
            <input id="uploader_name" name="uploader_name" className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm" />
          </div>
          <div className="grid gap-2">
            <label className="text-sm font-semibold" htmlFor="file">File</label>
            <input id="file" name="file" type="file" accept={ACCEPTED_UPLOAD_INPUT} required className="rounded-xl border border-dashed border-slate-300 bg-white px-3 py-3 text-sm" />
            <p className="text-xs text-slate-500">PDF, image, CSV, Excel, or Word files up to 25 MB.</p>
          </div>
          <button className="inline-flex h-11 items-center justify-center rounded-xl border border-emerald-700 bg-emerald-700 px-5 text-xs font-bold uppercase tracking-[0.09em] text-white shadow-[0_2px_0_#52271e] hover:-translate-y-px hover:bg-emerald-800" type="submit">Upload file</button>
        </form>
      </Card>
    </main>
  );
